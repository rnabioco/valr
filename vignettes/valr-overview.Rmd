...
bed_glyph(c, .fun = bed_merge, title_ops = "(x)")
```
...
Here we calculate a distribution of Jaccard statistics by shuffling `y` intervals compared to `x`. 

```{r jaccard_shuffle}
library(purrr)
```

We can use the same shuffled data to examine the relative distances between sets of intervals.
