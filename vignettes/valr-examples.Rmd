...
Next, we use `bed_map()` to map `y` signals onto the intersecting intervals in `x` and calculate summary statistics grouped by `win_id`. Note that `x` intervals with no `y` overlaps have values of `NA`, so `na.rm = TRUE` must be used during computation.
