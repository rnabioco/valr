...
bed_glyph(a, .fun = bed_cluster, max_dist = 20, 
  ...
  ggplot2::geom_label(aes((end - start) / 2 + start, bin + 0.5, label = .id)) + 
  xlab("") + ylab("")
```
...
bed_glyph(a, b, .fun = bed_map, value = sum(value.y), start_col = "start.x", end_col = "end.x", 
          title_ops = "(x, y, value = sum(value.y))") +
  geom_label(aes((end - start) / 2 + start, bin + 0.5, label = value)) + 
  xlab("") + ylab("")
```
