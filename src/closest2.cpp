...
bool ivl_overlaps(int xs, int xe, int ys, int ye) {
  return std::max(xs, ys) <= std::min(xe, ye);
}
...
//[[Rcpp::export]]
DataFrame closest_impl2(ValrGroupedDataFrame x, ValrGroupedDataFrame y,
  ...
  for (int i = 0; i < ng_x; i++) {
    ...
    IntegerVector before = findInterval(q_ends, sstr_rle.v) + 1;
    for(int j = 0; j < before.size(); j++){
      if(before[j] > sstr_rle.v.size()){
        before[j] = NA_INTEGER;
      } else {
        before[j] -= 1;
      }
    ...
    IntegerVector after = findInterval(q_starts, send_rle.v);
    ...
    for(int j = 0; j < nx; j++){
      ...
      if(min_is_before){
        ...
        idx = sstr_rle.s[ni];
        idx_l = sstr_rle.l[ni];
        bool ol = ivl_overlaps(q_starts[j], q_ends[j],
