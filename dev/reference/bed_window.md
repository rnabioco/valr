...
- y:
  ...
  is supplied, only the windowed regions around `x` are read from it
  (local files and `http(s)://` URLs are both supported), avoiding the
  cost of loading the entire file.
