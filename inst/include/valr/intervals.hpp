...
inline Coord interval_overlap(const Interval<Coord, Value>& a,
                               const Interval<Coord, Value>& b) {
...
struct IntervalStartCmp {
    bool operator()(const Interval<Coord, Value>& a,
                    const Interval<Coord, Value>& b) const {
...
private:
    ...
    void build_tree(interval_vector& intervals, size_t idx,
                    size_t lo, size_t hi) {
        if (lo >= hi || idx >= size_) return;
    ...
    void query_overlapping(size_t idx, Coord qstart, Coord qstop,
                           OutputIt& out) const {
        if (idx >= size_) return;

        // Prune: if max_stop in this subtree < query start, no overlaps possible
        if (max_stops_[idx] < qstart) return;
...
inline ivl_vector_t make_intervals(const cpp11::integers& starts,
                                   const cpp11::integers& ends,
