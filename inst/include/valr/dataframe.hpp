...
inline cpp11::writable::data_frame subset_dataframe(
    const cpp11::data_frame& df,
    ...
    for (int j = 0; j < ncol; ++j) {
        ...
        for (int i = 0; i < nrow_out; ++i) {
            ...
            switch (TYPEOF(new_col)) {
                case REALSXP:
                    REAL(new_col)[i] = (idx == NA_INTEGER) ?
                        NA_REAL : REAL(col)[idx];
                    break;
                case INTSXP:
                case LGLSXP:
                    INTEGER(new_col)[i] = (idx == NA_INTEGER) ?
                        NA_INTEGER : INTEGER(col)[idx];
                    break;
                case STRSXP:
                    SET_STRING_ELT(new_col, i,
                        (idx == NA_INTEGER) ? NA_STRING : STRING_ELT(col, idx));
                    break;
                case VECSXP:
                    SET_VECTOR_ELT(new_col, i,
                        (idx == NA_INTEGER) ? R_NilValue : VECTOR_ELT(col, idx));
                    break;
                default:
                    UNPROTECT(1);
...
public:
    ...
    void add_df(const cpp11::data_frame& df,
...
inline bool compare_rows(const cpp11::data_frame& df_x,
                         const cpp11::data_frame& df_y,
                         int idx_x, int idx_y) {
    int ncols = df_x.size();
